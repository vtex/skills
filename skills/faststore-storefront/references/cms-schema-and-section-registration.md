...
2. After every change to `cms/faststore/components/*.jsonc` or `cms/faststore/pages/*.jsonc`, you **must** publish the schema in the same working session (see [End-to-end agent workflow](#end-to-end-agent-workflow)). The **recommended** way is the consolidated **`faststore cms-sync`** (it runs `vtex content generate-schema` + `vtex content upload-schema` for you; install with `npm install -g @faststore/cli` if the binary is missing). If it is unavailable, fall back to running **`vtex content generate-schema`** and **`vtex content upload-schema`** manually. **Do not** use the legacy `yarn cms-sync` / `npm run cms-sync` project scripts (the v3-style full-sync behavior).
...
1. **Sync (recommended)** — run the consolidated command, which generates `cms/faststore/schema.json` and uploads it:
...
Add `--dry-run` to generate the schema without uploading.

2. **Validate** — for new or renamed sections, grep or read `cms/faststore/schema.json` and confirm the `"$componentKey"` is present.

3. **Manual fallback** (if `faststore cms-sync` is unavailable, or to run steps individually):
...
At the store ID prompt, enter the value of `contentSource.project` from `discovery.config.js` (the published id is `{account}.{project}`; NOT the hardcoded `faststore`). The logged-in account must still match `api.storeId` (see the account verification below).
...
> **`api.storeId` (account) vs the "store ID" prompt are different values.** The account verified above (`api.storeId` + `vtex whoami`) is the VTEX account you upload to. The value the CLI asks for at the **"store ID" prompt** is the CP project — the value of `contentSource.project` in `discovery.config.js` (falling back to `faststore` for legacy Headless CMS). The published schema id is `{account}.{project}`.

### Non-interactive upload (automatic - USE ONLY AFTER ACCOUNT VERIFICATION)

When uploading schema in an automated workflow, ALWAYS use `expect` to handle prompts automatically. **This block must only run after the pre-upload account verification above has passed and the user has confirmed.** It applies to the manual `upload-schema` fallback; the recommended path is `faststore cms-sync` (which prompts interactively for the same store ID).

```bash
# Derive the store ID prompt value from discovery.config.js:
# contentSource.project for CP, falling back to "faststore" for legacy Headless CMS.
