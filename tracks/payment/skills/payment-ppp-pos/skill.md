...
description: >
  Apply when implementing a VTEX Payment Provider Protocol connector for Point of Sale (POS)
  payments on VTEX Sales App. Covers Venda Direta Credito and Venda Direta Debito manifest methods,
  the two-phase Payment App challenge (terminal connector then wait-for-confirmation), null card
  data on Create Payment, asynchronous undefined status, timeout pairing of secondsWaiting and
  delayToCancel, and callback payloads with cardBrand, firstDigits, and lastDigits. Use for building
  or debugging PPP + POS integrations that charge a physical card on a payment terminal.
metadata:
  track: payment
  tags:
    - pos
  ...
  globs:
    ...
    - "**/pos/**/*.ts"
  version: "1.0"
  purpose: Scaffold a POS-capable PPP connector from the official PPF example so partners do not invent the Sales App payment flow
  applies_to:
    - building a payment connector for VTEX Sales App card-present POS
    - adding Venda Direta Credito or Venda Direta Debito to an existing connector
    - debugging terminal identification, wait-for-confirmation, or POS callbacks
  excludes:
    - ecommerce PPP endpoints with no POS methods (see payment-provider-protocol)
    - Pix, Boleto, and redirect async methods (see payment-async-flow)
    - ecommerce card PAN via Secure Proxy (see payment-pci-security)
    - Sales App cart, PDP, or menu UI extensions (see sales-app-extensibility)
  decision_scope:
    - POS card-present vs ecommerce card-not-present
    - built-in Payment Apps vs a custom Payment App
    - notification callback vs IO retry callback
  vtex_docs_verified: "2026-09-09"
---
