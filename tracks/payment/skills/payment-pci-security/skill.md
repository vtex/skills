...
description: >
  Apply when handling credit card data, implementing secureProxyUrl flows, or working with payment
  security and proxy code. Covers PCI DSS compliance, Secure Proxy card tokenization, sensitive data
  handling rules, X-PROVIDER-Forward-To header usage, custom token creation, and the constraint that
  Secure Proxy applies only to card authorization (not post-auth operations like cancel, capture, or
  refund). Use for any payment connector that processes credit, debit, or co-branded card payments
  to prevent data breaches and PCI violations.
metadata:
  track: payment
  tags:
    - pci-dss
  ...
  globs:
    ...
    - "**/secure/**/*.ts"
  version: "1.0"
  purpose: Ensure PCI DSS compliance and correct Secure Proxy usage for card payment connectors
  applies_to:
    - building payment connectors that accept credit/debit cards
    - routing acquirer calls through VTEX Secure Proxy
    - auditing card data handling for PCI compliance
  excludes:
  ...
  decision_scope:
    - Secure Proxy vs direct acquirer communication
    - what card data can vs cannot be stored
    - PCI-certified vs non-PCI environment requirements
