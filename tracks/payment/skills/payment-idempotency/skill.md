...
metadata:
  ...
  globs:
    ...
    - "**/connector/**/*.ts"
  version: "1.0"
  purpose: Prevent duplicate charges and ensure correct retry behavior across all PPP endpoints
  applies_to:
    - implementing Create Payment idempotency
    - handling Gateway retries for undefined payments
    - building Cancel/Capture/Refund with requestId guards
  excludes:
    - endpoint response shapes (see payment-provider-protocol)
    - async callback notification logic (see payment-async-flow)
    - card data handling (see payment-pci-security)
  decision_scope:
    - paymentId vs requestId as idempotency key
