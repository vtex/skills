...
name: ki-check
description: >
  Apply when checking whether the current repository or a given product area has open Known Issues (KIs)
  registered in Zendesk with tag:ki. Covers auto-detecting the app from git context or manifest.json,
  looking up a specific ticket by ID, and listing all open KIs for a product area ranked by fix complexity.
  Use before starting implementation work, during support triage, or when a customer reports a bug that
  may already be a tracked KI.
metadata:
  track: known-issues
  tags:
    - known-issues
    ...
    - triage
  version: "1.0"
  purpose: Surface open Known Issues relevant to the current codebase before starting implementation or support work
  applies_to:
    - checking for known bugs before implementing a fix
    - support triage against existing known issues
    - reviewing open KIs by product area or repository
    - pre-implementation research for a feature area
  excludes:
    - creating new KI tickets in Zendesk (use Zendesk directly)
    - looking up non-KI support tickets
    - real-time Zendesk API queries (this skill uses pre-processed local indexes)
  decision_scope:
    - repo-to-product-area-resolution
    - ki-complexity-assessment
  vtex_docs_verified: "2026-06-15"
---
