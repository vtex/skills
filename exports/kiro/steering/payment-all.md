...
- POS / VTEX Sales App terminal payments (`Venda Direta Credito`, `Venda Direta Debito`, Payment Apps) — use [`payment-ppp-pos`](payment-payment-ppp-pos.md)
...
- **Wrong `this.context` access for building callback URLs** — Using `this.context.account` instead of `this.context.vtex.account` in the PPF. See the [`payment-provider-framework`](payment-payment-provider-framework.md) skill for the correct IOContext access pattern.
...
- [`payment-provider-framework`](payment-payment-provider-framework.md) — PPF IO wiring, `service.json`, custom routes, IOContext access, and `PaymentProviderService` configuration
...
- [`payment-ppp-pos`](payment-payment-ppp-pos.md) — POS / Sales App terminal flow; not Pix/Boleto
...
- POS / Sales App terminal payments (`Venda Direta Credito`, `Venda Direta Debito`, overlapping Create Payment while Wait for confirmation polls) — use [`payment-ppp-pos`](payment-payment-ppp-pos.md) together with this skill
...
- If the Gateway sends a second Create Payment with the same `paymentId`, return the stored response without calling the acquirer again. Those retries can **overlap**; last-write-wins storage can drop a status update — persist with an etag / `ifMatch` (see [`payment-ppp-pos`](payment-payment-ppp-pos.md) for the POS overlay).
...
- [`payment-pci-security`](payment-payment-pci-security.md) — PCI compliance and Secure Proxy
- [`payment-ppp-pos`](payment-payment-ppp-pos.md) — POS Create Payment is polled and can overlap for one `paymentId`; use this skill's `paymentId` key plus that skill's etag / `ifMatch` pattern
...
- [`payment-provider-framework`](payment-payment-provider-framework.md) — PPF IO wiring, two-client pattern (`SecureExternalClient` + `ExternalClient`), and `outbound-access` policies
- [`payment-provider-protocol`](payment-payment-provider-protocol.md) — Endpoint contracts and response shapes
- [`payment-idempotency`](payment-payment-idempotency.md) — `paymentId`/`requestId` idempotency and state machine
...
- Ecommerce PPP endpoints, manifests, and homologation shapes with no POS methods — use [`payment-provider-protocol`](payment-payment-provider-protocol.md)
- IO connector wiring (`PaymentProvider`, `configuration.json`, `this.retry`) — use [`payment-provider-framework`](payment-payment-provider-framework.md) together with this skill
- Pix, Boleto, or browser-redirect async methods — use [`payment-async-flow`](payment-payment-async-flow.md)
- Ecommerce card data and Secure Proxy — use [`payment-pci-security`](payment-payment-pci-security.md)
- Sales App cart, PDP, or menu UI extensions — use [`sales-app-extensibility`](sales-app-sales-app-extensibility.md)
...
On PPF, persist with `getRawJSON` (capture `etag`) then `saveJSON(..., ifMatch)`. On 412, re-read and continue from the stored document. Pair with [`payment-idempotency`](payment-payment-idempotency.md). Do not put unique per-request fields (random `tid`, `requestId`) on the first identify-terminal write.
...
- Applying [`payment-pci-security`](payment-payment-pci-security.md) Secure Proxy to POS authorize because "it is a card payment".
...
- [`payment-provider-protocol`](payment-payment-provider-protocol.md) — Mandatory PPP endpoints and response shapes; use together with this skill
- [`payment-provider-framework`](payment-payment-provider-framework.md) — Use when the POS connector is a VTEX IO app
- [`payment-async-flow`](payment-payment-async-flow.md) — Generic `undefined` + `callbackUrl` rules; POS adds Payment Apps and card-fragment callbacks on top
- [`payment-idempotency`](payment-payment-idempotency.md) — `paymentId` must not create a second POS charge; Create Payment for one id can overlap (etag / `ifMatch`)
- [`payment-pci-security`](payment-payment-pci-security.md) — Ecommerce card-not-present only; do not apply Secure Proxy to POS authorize
- [`sales-app-extensibility`](sales-app-sales-app-extensibility.md) — Sales App UI extensions; not the POS payment connector
...
- POS / Sales App card-present overlay (`Venda Direta` methods, Payment Apps, terminal serial) — use [`payment-ppp-pos`](payment-payment-ppp-pos.md) together with this skill
...
- [`payment-async-flow`](payment-payment-async-flow.md) — `undefined` status, `callbackUrl` (IO retry vs notification), and redirect-based flows
- [`payment-pci-security`](payment-payment-pci-security.md) — PCI and Secure Proxy semantics beyond IO wiring
- [`payment-ppp-pos`](payment-payment-ppp-pos.md) — POS / Sales App card-present overlay (`Venda Direta` methods, Payment Apps, terminal callbacks)
...
- PCI compliance and Secure Proxy card handling — use [`payment-pci-security`](payment-payment-pci-security.md)
- POS / VTEX Sales App card-present payments (`Venda Direta Credito`, `Venda Direta Debito`, terminal Payment Apps) — use [`payment-ppp-pos`](payment-payment-ppp-pos.md) together with this skill for endpoint shapes
...
- [`payment-ppp-pos`](payment-payment-ppp-pos.md) — POS / Sales App card-present overlay; use with this skill for endpoint shapes
