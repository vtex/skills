...
|---|---|---|---|
| 1 | Connector name (`vendor.app-name`) | Becomes `manifest.json` `vendor`/`name` and the Admin affiliation. VBase metadata is `{vendor}.{appName}.{bucket}` and must stay ≤ 50 characters — keep the POS bucket `pos`. | `dummyaccount.dummy-latam-pos` |
...
2. Persist a state machine keyed by `paymentId`: `identify-terminal` → `await-pos` → `approved` | `denied` | `canceled`. On PPF, use VBase (`vbase-read-write`) with a **short** bucket (`pos`) and `getRawJSON` + `saveJSON(..., ifMatch)` — Create Payment calls for one `paymentId` can overlap. Store `callbackUrl`, `paymentMethod`, and `value` on first authorize. Do not invent a random `tid` on that first write.
3. First `authorize()`: `Authorizations.pending` + `vtex.terminal-connector-app` with string `payload.submitUrl`. **STOP** if `payload` is a nested object, `appName` is `vtex.challenge-terminal-connector-app`, or the pending body is cast with `as AuthorizationResponse`.
4. Public HTTPS route for `{"serialNumber":"..."}`. Persist the serial only (POS guide step 7). **Not** PPP inbound-request. **STOP** if `submitUrl` contains `/inbound` or if this handler starts the POS charge.
5. The **next** Create Payment (Sales App after the terminal app closes, step 8) starts the processor charge **once** (`ifMatch` winner only). Repeated polls (Wait for confirmation, step 9) must not start a second charge and must not POST `callbackUrl`.
6. That Create Payment returns `undefined` + `vtex.challenge-wait-for-confirmation` (`secondsWaiting`). `delayToCancel >= secondsWaiting`.
7. Processor webhook:
   - **PPF extra route:** persist final status + `cardBrand` / `firstDigits` / `lastDigits`, then `new Payments(ctx.vtex).retry(stored.callbackUrl)` (bodiless POST; keep the query string / `X-VTEX-signature`). That is IO retry. `this.retry(authorization)` is only valid inside a `PaymentProvider` method, not in a standalone route handler.
...
- Whether the partner is on IO retry (bodiless `Payments.retry` / `this.retry`) or standalone notification (status + card fragments)
