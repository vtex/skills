...
| [references/cms-schema-and-section-registration.md](faststore-storefront/references/cms-schema-and-section-registration.md)         | VTEX Headless CMS: `cms_component__*.jsonc` + `index.tsx` as source of truth, generated `schema.json`, end-to-end `vtex content` (no legacy `cms-sync`), mandatory `upload-schema`, Admin → Content vs `pages` JSONC, scopes, CMS props only (no ad-hoc props)                            |
